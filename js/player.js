...
function updateHUD() {
  ...
  document.getElementById('highScoreVal').textContent = inPracticeSession ? '—' : (gameMode === 'sprint' ? highScoreSprint : currentWorld === 2 ? highScoreWorld2 : (gameMode === 'hardcore' ? highScoreHardcore : highScore));
  ...
  if (player.activeTransform === 'tank') {
    ...
    if (['combofire', 'combofrost', 'combovolt', 'comboneonpink', 'comboneoncyan', 'comboneonlime', 'comboearth', 'combowind', 'combowater', 'combocrystal'].includes(equippedSkin) && player.comboStreak > 0) active.push(`🔥 Combo x${player.comboStreak}`);
