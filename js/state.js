...
const ARMOR = [
  ...
  { id: 'juggernaut', name: 'Juggernaut',          price: 950,  hpBonus: 120, reduction: 0.4,  desc: '+120 max HP, -40% inkomende schade.' },
  { id: 'regen',      name: 'Regeneratie-pantser', price: 750,  hpBonus: 20,  reduction: 0,    regen: 2,        desc: '+20 max HP. Geneest passief 2 HP per seconde.' },
...
function getWeapon() {
  ...
  // Wapens van de ene wereld werken niet in de andere wereld — val dan terug op het standaard pistool
  if (currentWorld === 2 && SPECIAL_WEAPONS.some(sw => sw.id === w.id)) return WEAPONS[0];
  if (currentWorld !== 2 && (WORLD2_SPECIAL_WEAPONS.some(sw => sw.id === w.id) || WORLD2_WEAPONS.some(sw => sw.id === w.id))) return WEAPONS[0];
  return w;
}
function getArmor() {
