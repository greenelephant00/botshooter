...
const ACHIEVEMENTS = [
  { id: 'first_kill', category: 'w1', name: 'Eerste Bloed', icon: '🎯', desc: 'Dood je eerste bot.', reward: { type: 'coins', amount: 50 }, check: () => score > 0 || highScore > 0 || highScoreWorld2 > 0 || highScoreHardcore > 0 },
  { id: 'first_boss', category: 'w1', name: 'Bazenslachter', icon: '⚔️', desc: 'Versla je eerste boss.', reward: { type: 'coins', amount: 150 }, check: () => hasFirstBoss },
  { id: 'score_1000', category: 'w1', name: 'Op Gang', icon: '⭐', desc: 'Behaal een score van 1000 in één potje.', reward: { type: 'coins', amount: 100 }, check: () => highScore >= 1000 || highScoreWorld2 >= 1000 || highScoreHardcore >= 1000 },
  { id: 'score_5000', category: 'w1', name: 'Doorgewinterd', icon: '🌟', desc: 'Behaal een score van 5000 in één potje.', reward: { type: 'coins', amount: 300 }, check: () => highScore >= 5000 || highScoreWorld2 >= 5000 || highScoreHardcore >= 5000 },
  { id: 'score_10000', category: 'w1', name: 'Veteraan', icon: '💫', desc: 'Behaal een score van 10.000 in één potje.', reward: { type: 'coins', amount: 600 }, check: () => highScore >= 10000 || highScoreWorld2 >= 10000 || highScoreHardcore >= 10000 },
  { id: 'score_25000', category: 'w1', name: 'Legende', icon: '👑', desc: 'Behaal een score van 25.000 in één potje.', reward: { type: 'coins', amount: 1200 }, check: () => highScore >= 25000 || highScoreWorld2 >= 25000 || highScoreHardcore >= 25000 },
  { id: 'hardcore_2500', category: 'w1', name: 'Hardcore Overlever', icon: '☠️', desc: 'Behaal een score van 2500 in Hardcore.', reward: { type: 'coins', amount: 500 }, check: () => highScoreHardcore >= 2500 },
  { id: 'world2_unlocked', category: 'w2', name: 'Elementair', icon: '🔥', desc: 'Ontgrendel Wereld 2.', reward: { type: 'coins', amount: 500 }, check: () => world2Unlocked },
  { id: 'bossrush_w1', category: 'w1', name: 'Eindbaas Rush: Wereld 1', icon: '🏆', desc: 'Voltooi de Eindbaas Rush in Wereld 1.', reward: { type: 'coins', amount: 400 }, check: () => hasBossRushW1 },
  { id: 'bossrush_w2', category: 'w2', name: 'Eindbaas Rush: Wereld 2', icon: '🏆', desc: 'Voltooi de Eindbaas Rush in Wereld 2.', reward: { type: 'cores', amount: 20 }, check: () => hasBossRushW2 },
  { id: 'worldboss', category: 'w2', name: 'Oerelementaal Verslagen', icon: '🌍', desc: 'Versla de Oerelementaal, de zeldzame wereldbaas van Wereld 2.', reward: { type: 'cores', amount: 40 }, check: () => hasWorldBoss },
  { id: 'cores_10', category: 'w2', name: 'Kernverzamelaar', icon: '🔮', desc: 'Verzamel 10 Elemental Cores.', reward: { type: 'coins', amount: 100 }, check: () => elementalCores >= 10 },
  { id: 'cores_50', category: 'w2', name: 'Kernmeester', icon: '💎', desc: 'Verzamel 50 Elemental Cores.', reward: { type: 'coins', amount: 300 }, check: () => elementalCores >= 50 },
  { id: 'all_transforms', category: 'w1', name: 'Gedaanteverwisselaar', icon: '🔄', desc: 'Ontgrendel alle Wereld 1-transformaties.', reward: { type: 'coins', amount: 800 }, check: () => TRANSFORMS.every(t => ownedTransforms.includes(t.id)) },
  { id: 'all_weapons_w1', category: 'w1', name: 'Wapenverzamelaar', icon: '🔫', desc: 'Bezit alle Wereld 1-wapens (normaal en speciaal).', reward: { type: 'coins', amount: 500 }, check: () => [...WEAPONS, ...SPECIAL_WEAPONS].every(w => ownedWeapons.includes(w.id)) },
  { id: 'all_armor_w1', category: 'w1', name: 'Pantserverzamelaar', icon: '🛡️', desc: 'Bezit alle Wereld 1-pantsers.', reward: { type: 'coins', amount: 500 }, check: () => ARMOR.every(a => ownedArmor.includes(a.id)) },
  { id: 'all_world2_weapons', category: 'w2', name: 'Elementwapens', icon: '🔥', desc: 'Bezit alle Wereld 2-wapens (normaal en speciaal, exclusief Kern-winkel).', reward: { type: 'cores', amount: 30 }, check: () => [...WORLD2_WEAPONS, ...WORLD2_SPECIAL_WEAPONS].filter(w => !w.coreOnly).every(w => ownedWeapons.includes(w.id)) },
  { id: 'all_world2_armor', category: 'w2', name: 'Elementpantsers', icon: '🪨', desc: 'Bezit alle Wereld 2-pantsers (exclusief Kern-winkel).', reward: { type: 'cores', amount: 30 }, check: () => WORLD2_ARMOR.filter(a => !a.coreOnly).every(a => ownedArmor.includes(a.id)) },
  { id: 'dual_armor', category: 'w1', name: 'Dubbel Gepantserd', icon: '🛡️', desc: 'Koop het 2e pantser-slot.', reward: { type: 'coins', amount: 300 }, check: () => hasDualArmor },
  { id: 'coins_5000', category: 'w1', name: 'Spaarpot', icon: '🪙', desc: 'Heb ooit 5000 munten tegelijk in bezit.', reward: { type: 'cores', amount: 10 }, check: () => coins >= 5000 },
  { id: 'coins_20000', category: 'w1', name: 'Rijkdom', icon: '💰', desc: 'Heb ooit 20.000 munten tegelijk in bezit.', reward: { type: 'cores', amount: 25 }, check: () => coins >= 20000 },
  { id: 'all_skins_normal', category: 'w1', name: 'Modebewust', icon: '🎨', desc: 'Bezit alle gewone skins (geen killstreak- of elementen-skins).', reward: { type: 'coins', amount: 500 }, check: () => SKINS.filter(s => !s.killstreak && !s.element && !s.coreOnly).every(s => ownedSkins.includes(s.id)) },
  { id: 'all_killstreak_skins', category: 'w1', name: 'Streak Style', icon: '🔥', desc: 'Bezit alle Wereld 1-killstreak-skins.', reward: { type: 'coins', amount: 400 }, check: () => SKINS.filter(s => s.killstreak && !s.element).every(s => ownedSkins.includes(s.id)) },
  { id: 'all_element_skins', category: 'w2', name: 'Elementenmode', icon: '🌍', desc: 'Bezit alle gewone Elementen Skins.', reward: { type: 'cores', amount: 25 }, check: () => SKINS.filter(s => s.element && !s.killstreak && !s.coreOnly).every(s => ownedSkins.includes(s.id)) },
  { id: 'all_element_killstreak_skins', category: 'w2', name: 'Elementaire Streak', icon: '🌈', desc: 'Bezit alle Wereld 2-killstreak-elementen-skins.', reward: { type: 'cores', amount: 30 }, check: () => SKINS.filter(s => s.element && s.killstreak).every(s => ownedSkins.includes(s.id)) },
  { id: 'core_shop_full', category: 'w2', name: 'Kern-verzamelaar', icon: '🔮', desc: 'Bezit Kernpantser, Kernblaster en Kernwezen.', reward: { type: 'coins', amount: 500 }, check: () => ownedArmor.includes('coreplate') && ownedWeapons.includes('coreblaster') && ownedSkins.includes('coreessence') },
  { id: 'max_upgrade_w1', category: 'w1', name: 'Volledig Uitgerust', icon: '💪', desc: 'Bereik het maximale niveau van Extra Conditie.', reward: { type: 'coins', amount: 300 }, check: () => lvlExtraHp >= EXTRA_HP_LEVELS.length },
  { id: 'max_core_damage', category: 'w2', name: 'Kernkracht Voltooid', icon: '💥', desc: 'Bereik het maximale niveau van Kernkracht.', reward: { type: 'cores', amount: 20 }, check: () => lvlCoreDamage >= CORE_DAMAGE_LEVELS.length },
  { id: 'max_core_shield', category: 'w2', name: 'Kernschild Voltooid', icon: '🛡️', desc: 'Bereik het maximale niveau van Kernschild.', reward: { type: 'cores', amount: 20 }, check: () => lvlCoreShield >= CORE_SHIELD_LEVELS.length },
  { id: 'core_harvest_owned', category: 'w2', name: 'Kernoogster', icon: '🌾', desc: 'Koop Kernoogst.', reward: { type: 'cores', amount: 15 }, check: () => hasCoreHarvest },
  { id: 'revive_w1', category: 'w1', name: 'Reanimatie Gekocht', icon: '❤️', desc: 'Koop Reanimatie in Wereld 1.', reward: { type: 'coins', amount: 200 }, check: () => hasRevive },
  { id: 'revive_w2', category: 'w2', name: 'Elementreanimatie Gekocht', icon: '💚', desc: 'Koop Elementreanimatie in Wereld 2.', reward: { type: 'cores', amount: 15 }, check: () => hasRevive2 },
  { id: 'highlevel_20', category: 'w1', name: 'Klimmer', icon: '🧗', desc: 'Bereik level 20 in Levels-modus.', reward: { type: 'coins', amount: 400 }, check: () => highLevel >= 20 },
  { id: 'highlevel_40', category: 'w1', name: 'Bergbeklimmer', icon: '⛰️', desc: 'Bereik level 40 in Levels-modus.', reward: { type: 'coins', amount: 800 }, check: () => highLevel >= 40 },
  { id: 'max_goldrush', category: 'w1', name: 'Muntenmagnaat', icon: '🧲', desc: 'Bereik het maximale niveau van Goudtrek.', reward: { type: 'coins', amount: 300 }, check: () => lvlGoldRush >= GOLD_RUSH_LEVELS.length },
  { id: 'max_criticalhit', category: 'w1', name: 'Dodelijke Precisie', icon: '🎯', desc: 'Bereik het maximale niveau van Kritieke Hit.', reward: { type: 'coins', amount: 300 }, check: () => lvlCriticalHit >= CRITICAL_HIT_LEVELS.length },
  { id: 'score_50000', category: 'w1', name: 'Onsterfelijk', icon: '♾️', desc: 'Behaal een score van 50.000 in één potje.', reward: { type: 'coins', amount: 2000 }, check: () => highScore >= 50000 || highScoreWorld2 >= 50000 || highScoreHardcore >= 50000 },
  { id: 'highlevel_60', category: 'w1', name: 'Levelloper', icon: '🏔️', desc: 'Bereik level 60 in Levels-modus.', reward: { type: 'coins', amount: 1200 }, check: () => highLevel >= 60 },
  { id: 'max_secondwind', category: 'w1', name: 'IJzeren Wil Meester', icon: '🩹', desc: 'Bereik het maximale niveau van IJzeren Wil.', reward: { type: 'coins', amount: 300 }, check: () => lvlSecondWind >= SECOND_WIND_LEVELS.length },
  { id: 'max_core_speed', category: 'w2', name: 'Kernsnelheid Voltooid', icon: '💨', desc: 'Bereik het maximale niveau van Kernsnelheid.', reward: { type: 'cores', amount: 20 }, check: () => lvlCoreSpeed >= CORE_SPEED_LEVELS.length },
  { id: 'max_core_regen', category: 'w2', name: 'Kernregeneratie Voltooid', icon: '💗', desc: 'Bereik het maximale niveau van Kernregeneratie.', reward: { type: 'cores', amount: 20 }, check: () => lvlCoreRegen >= CORE_REGEN_LEVELS.length },
  { id: 'max_core_vampire', category: 'w2', name: 'Kernvampier Voltooid', icon: '🧛', desc: 'Bereik het maximale niveau van Kernvampier.', reward: { type: 'cores', amount: 20 }, check: () => lvlCoreVampire >= CORE_VAMPIRE_LEVELS.length },
  { id: 'core_aura_owned', category: 'w2', name: 'Kernaura Ontgrendeld', icon: '💫', desc: 'Koop Kernaura.', reward: { type: 'cores', amount: 15 }, check: () => hasCoreAura },
  { id: 'core_shock_owned', category: 'w2', name: 'Kernschok Ontgrendeld', icon: '⚡', desc: 'Koop Kernschok.', reward: { type: 'cores', amount: 15 }, check: () => hasCoreShock }
];
...
const SKINS = [
  ...
  { id: 'comboearth',  name: 'Aardschok Combo',   price: 1000, desc: 'Wereld 2-exclusief killstreak-skin. Dof gesteente dat openbarst en gaat gloeien met mos-groene aders, met steeds meer rotspieken naarmate je killstreak oploopt. Werkt alleen in Wereld 2.', killstreak: true, element: true },
